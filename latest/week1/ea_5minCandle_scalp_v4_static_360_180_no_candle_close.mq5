//+------------------------------------------------------------------+
//| ea_5minCandle_scalp_v4_static_360_180_no_candle_close.mq5       |
...
//+------------------------------------------------------------------+
//| M5 Candle Scalper v4.2 — Static SL/TP Only                       |
//| Static stop loss 360 points and take profit 180 points           |
//| Dynamic, dollar, and candle-end forced close logic removed       |
//+------------------------------------------------------------------+
...
input int StaticStopLossPoints = 360;
input int StaticTakeProfitPoints = 180;

//=== #3 HOLD WINNERS PAST CANDLE (kept for compatibility, not used for close) ===
...
int OnInit()
{
   ...
   Print("[INIT] M5 Candle Scalper v4.2 (Static SL/TP Only)");
   Print("[INIT] Static SL=" + IntegerToString(StaticStopLossPoints) + "pts | TP=" + IntegerToString(StaticTakeProfitPoints) + "pts");
...
void OnTick()
{
   ...
   if(HasPosition() && UseTrailingStop)
      ManageTrailing();
...
{
   sl_dist = StaticStopLossPoints * _Point;
   tp_dist = StaticTakeProfitPoints * _Point;
   Print("[SLTP] Static SL=", IntegerToString(StaticStopLossPoints), "pts | TP=", IntegerToString(StaticTakeProfitPoints), "pts");
}
