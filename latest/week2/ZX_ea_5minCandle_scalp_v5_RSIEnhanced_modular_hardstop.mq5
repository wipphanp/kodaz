//+------------------------------------------------------------------+
//| ZX_ea_5minCandle_scalp_v5_RSIEnhanced_modular_hardstop.mq5      |
//| Modular RSI-enhanced EA with Loss Prevention Hard Stop           |
//+------------------------------------------------------------------+
...
input string OpenAI_ApiKey = "sk-proj-";
...
input double HardStop_LossDollars = 81.0;
...
// Loss Prevention Criterion : Hard Stop
void CheckHardStopLoss()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(profit <= -HardStop_LossDollars)
      {
         if(trade.PositionClose(ticket))
         {
            TodayLosses++;
            TradeOpenThisCandle = false;
            Print("[HARD STOP] Closed losing trade at $", DoubleToString(profit, 2));
         }
      }
   }
}
