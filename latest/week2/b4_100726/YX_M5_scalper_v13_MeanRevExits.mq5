//+------------------------------------------------------------------+
//|                              YX_M5_scalper_v13_MeanRevExits.mq5 |
...
//+------------------------------------------------------------------+
//|  #13 DEDICATED MEAN-REVERSION SL/TP (NEW, v3.08 / v13):              |
//|     Even after #12's direction fix, every mean-reversion trade still |
//|     shared GetSLTP() with momentum trades - the same wide            |
//|     Fixed_SL_Points (1503pt) backstop and the same large RR/ATR-     |
//|     linked TP built for trend-continuation moves. A reversion trade  |
//|     is a different bet (a fast snap back toward the mean, with quick |
//|     invalidation if the market keeps extending instead), so it now   |
//|     gets its own ATR-relative SL/TP:                                 |
//|        MeanRev TP = clamp(ATR_pts * MeanRev_TP_ATR_Mult,             |
//|                            MeanRev_Min_TP_Points, MeanRev_Max_TP_Points) |
//|        MeanRev SL = clamp(ATR_pts * MeanRev_SL_ATR_Mult,             |
//|                            MeanRev_Min_SL_Points, MeanRev_Max_SL_Points) |
//|     Implementation: GetSLTP() gained an isMeanRev parameter (default |
//|     false = 100% unchanged trend-engine path). ExecuteBuy()/         |
//|     ExecuteSell() gained the same isMeanRev parameter (default       |
//|     false) and pass it straight through; only the two mean-reversion |
//|     entry calls inside CheckM1Confirmation() now pass true - the     |
//|     momentum-confirmation branch and the ladder subsystem's calls    |
//|     are untouched and still default to false, so their SL/TP is      |
//|     byte-for-byte identical to v12/v11. Mean-reversion trades are    |
//|     also tagged with a distinct order comment ("M5v4 BUY MR" /       |
//|     "M5v4 SELL MR") purely so they're identifiable in trade history  |
//|     - IsExcludedComment() only matches "RECOVERY_"/"LADDER_HEDGE_"   |
//|     prefixes, so these trades still flow through breakeven/trailing/ |
//|     candle-end-close exactly like any other trade.                   |
//|     Set UseMeanRev_DedicatedExits=false to fall back to sharing      |
//|     GetSLTP() with momentum trades exactly like v12 did.             |
//+------------------------------------------------------------------+
...
//=== #13 MEAN-REVERSION DEDICATED SL/TP (NEW, v13) ===
// v12 gave mean reversion a correct direction/trigger filter, but every
// trade - mean-reversion or momentum - still shared the exact same
// GetSLTP() output: the wide Fixed_SL_Points backstop (1503pts) and the
// large RR/ATR-linked TP built for trend-continuation trades. A reversion
// trade is a different bet (fast snap back toward the mean, quick
// invalidation if wrong) and deserves its own, tighter, ATR-relative
// SL/TP instead of the trend engine's numbers. Set UseMeanRev_DedicatedExits
// = false to fall back to sharing GetSLTP() with momentum trades exactly
// like v12/v11 did.
...
int OnInit()
{
   ...
   Print("[INIT] Mean Reversion dedicated exits: enabled=" + (UseMeanRev_DedicatedExits ? "true" : "false") +
         (UseMeanRev_DedicatedExits ?
            ...
            : " | mean-rev trades share the trend engine's SL/TP (same as v12)"));
      ...
      if(CopyBuffer(hRSI_M1, 0, 1, 1, rsiM1) > 0 && CopyBuffer(hRSI_M5, 0, m5Shift, 1, rsiM5) > 0)
      {
            ...
            m1AbsBody >= MeanRev_MinM1Body)
         {
            ...
            else
            {
               ...
               ExecuteSell(-1, true);  // #13 v13: isMeanRev=true -> dedicated ATR-relative SL/TP
               return;
            }
         }
            ...
            m1AbsBody >= MeanRev_MinM1Body)
         {
            ...
            else
            {
               Print("[MEAN REVERSAL] BUY | M1 RSI=", DoubleToString(m1RsiVal, 1),
                     " | M5 RSI=", DoubleToString(m5RsiVal, 1),
                     " | M1 body=", DoubleToString(m1AbsBody, 1), "pts");
               ExecuteBuy(-1, true);  // #13 v13: isMeanRev=true -> dedicated ATR-relative SL/TP
               return;
            }
         }
      }
...
//+------------------------------------------------------------------+
// #13 v13: isMeanRev (default false) lets a mean-reversion entry request its
// own ATR-relative SL/TP instead of the trend engine's Fixed_SL_Points/RR-
// or-ATR TP. Momentum trades (isMeanRev=false, the default) go through the
// exact same code path as before - nothing about that branch changed.
...
// isMeanRev (v13, NEW): false (default) means "use GetSLTP()'s normal
// trend-engine SL/TP exactly as before" - momentum entries and every
// existing caller (ladder, etc.) are completely unaffected. Only the
// mean-reversion branch in CheckM1Confirmation() passes true, which routes
// GetSLTP() to the dedicated ATR-relative mean-reversion SL/TP instead.
void ExecuteBuy(double lotOverride = -1, bool isMeanRev = false)
{
...
//+------------------------------------------------------------------+
// isMeanRev (v13, NEW): see the note above ExecuteBuy() - identical pattern.
void ExecuteSell(double lotOverride = -1, bool isMeanRev = false)
{
   double sl_dist, tp_dist;
   GetSLTP(sl_dist, tp_dist, isMeanRev);
   double lot = (lotOverride > 0) ? lotOverride : CalcLot(sl_dist);
