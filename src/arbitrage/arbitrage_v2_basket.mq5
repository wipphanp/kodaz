//+------------------------------------------------------------------+
//| arbitrage_v2_basket.mq5                                          |
//| Spot-Futures Basis / Statistical Arbitrage EA - Basket TP Edition|
//| Based on arbitrage_v2_improved.mq5 with the following change:    |
//|   - Removed ALL individual per-pair SL/TP, std-based target/stop,|
//|     and z-score partial profit-taking.                           |
//|   - Added a BASKET take-profit / stop-loss that evaluates the    |
//|     combined floating P/L of every open leg under this EA's      |
//|     Magic number and closes everything at once when the target   |
//|     (or optional loss limit) is hit.                             |
//|   - Individual pairs now only close via: basket TP/SL, MaxHoldH  |
//|     time failsafe, futures-expiry lock, or orphan-leg sweep.      |
//+------------------------------------------------------------------+
...
input bool   ScaleSecondLeg = true;  // volatility-based dollar-neutral hedge
input double MaxRatioB      = 3.0;

input group "=== Basket Management (NEW - replaces individual SL/TP) ==="
input double BasketTP       = 50.0;  // close ALL positions when combined floating P/L >= this (account currency)
input double BasketSL       = 0.0;   // close ALL positions when combined floating P/L <= -this (0 = disabled)
input int    BasketCheckSec = 5;     // how often to evaluate basket P/L
input int    MaxHoldH       = 60;    // safety time-based failsafe per pair (NOT a TP/SL)
input int    CleanupSec     = 15;

input group "=== Control ==="
input int    MaxFails     = 10;
input int    StaleTickSec = 20;
input long   Magic        = 246814;
input int    Slippage     = 30;
input string Tag          = "WPV2";
input bool   DebugLog     = true;

//============================ GLOBALS ===============================
double   g_ref = 0.0; bool g_refInit = false;
double   g_spreadVar = 0.0;
...
bool RelationshipHealthy()
{
   ...
   if(g_histCount < MathMin(30, CorrWindow)) return true;
...
bool Ok(string sym)
{
   return(SymbolInfoInteger(sym,SYMBOL_TRADE_MODE) == SYMBOL_TRADE_MODE_FULL);
}
...
void Open(bool mainMode, double lotA, double refval, double zscore)
{
   ...
   double lotB = LotB(lotA);
   uint ret=0; ulong deal=0;
...
void Sweep()
{
   ...
   for(int i=0;i<n;i++)
   {
      int legs=0; datetime oldest=0;
      LegInfo(ids[i], legs, oldest);
      if(legs==1 && oldest>0 && (TimeCurrent()-oldest) >= CleanupSec)
      {
...
//+------------------------------------------------------------------+
// BASKET TP/SL (replaces all individual per-pair SL/TP, std-based
// target/stop, and z-score partial profit-taking from v2).
//+------------------------------------------------------------------+
double BasketProfit()
{
   double sum = 0.0;
...
{
   datetime now = TimeCurrent();
   if((now - g_lastBasketCheck) < BasketCheckSec) return;
   g_lastBasketCheck = now;

   if(Count()==0) return;

   double bp = BasketProfit();
   if(BasketTP>0 && bp>=BasketTP)
   {
      if(DebugLog) PrintFormat("[BASKET TP] closing all - profit=%.2f target=%.2f", bp, BasketTP);
      CloseAllBasket();
   }
   else if(BasketSL>0 && bp<=-BasketSL)
   {
      if(DebugLog) PrintFormat("[BASKET SL] closing all - loss=%.2f limit=%.2f", bp, BasketSL);
      CloseAllBasket();
   }
}

//+------------------------------------------------------------------+
// Manage(): NO individual SL/TP. Only safety failsafes remain:
//   - MaxHoldH  : force-close a pair that has been open too long
//   - Locked()  : force-close everything ahead of futures expiry
//+------------------------------------------------------------------+
void Manage()
{
   int ids[]; int n = Ids(ids);
   ...
   {
      int id = ids[i];
      datetime ot = OldestTime(id);

      bool t3 = (MaxHoldH>0 && ot>0 && (TimeCurrent()-ot) >= MaxHoldH*3600);

      if(t3 || lock)
      {
         if(DebugLog) PrintFormat("[CLOSE] id=%d reason=%s", id, lock?"locked":"timeout");
         CloseId(id);
      }
   }
}
...
void LegInfo(int id, int &legs, datetime &oldest)
{
   legs=0; oldest=0;
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
...
datetime OldestTime(int id)
{
   int legs; datetime oldest;
   LegInfo(id, legs, oldest);
   return(oldest);
}
