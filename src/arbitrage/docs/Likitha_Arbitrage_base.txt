...
input string   SymA            = "XAUUSD.na";      // leg A
input string   SymB            = "GOLD_AUG26.na";  // leg B
...
void Open(bool mainMode,double lotA,double refval)
{
   ...
   // NB: a rare "done-then-rejected" broker reversal leaves one leg alone;
   //     the orphan sweep (CleanupSec) closes it. No synchronous position
   //     check here - it races the deal confirmation and blocks the spot leg.
}
...
double LotB(double lotA){
   ...
   if(b<=0||a<=0) return(lotA);
   double v=lotA*a/b, cap=lotA*MaxRatioB; if(v>cap)v=cap; return(v); }
