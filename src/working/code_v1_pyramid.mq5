...
input int Scalp_CooldownSeconds = 45;     // Cooldown between entries
input int Scalp_MaxTradesPerDay = 20;     // Max total trades per day
input double Scalp_MaxSpreadPoints = 30.0;
input int EMA_Fast = 9;
input int EMA_Slow = 21;

//=== PYRAMIDING (Multiple Entries in Trend) ===
input bool UsePyramiding = true;          // Allow multiple positions in same direction
input int MaxOpenPositions = 3;           // Maximum simultaneous positions
input int Pyramid_CooldownSeconds = 60;   // Minimum seconds between pyramid entries
input double Pyramid_MinProfitPoints = 40; // Existing position must be in profit before adding
input bool CloseAllOnReversal = true;     // Close ALL positions when AI bias reverses
...
string AI_Bias = "HOLD";
string AI_Bias_Previous = "HOLD";       // Previous bias for reversal detection
int AI_Confidence = 0;
...
void OnTick()
{
   ...
   // === REVERSAL DETECTION: Close all positions on bias flip ===
   if(CloseAllOnReversal && HasPosition())
   {
      ENUM_POSITION_TYPE posType = CurrentPositionType();
      // AI changed from BUY to SELL (or vice versa) = reversal
      if((posType == POSITION_TYPE_BUY && AI_Bias == "SELL") ||
         (posType == POSITION_TYPE_SELL && AI_Bias == "BUY"))
      {
         int posCount = CountOpenPositions();
         Print("[REVERSAL] AI Bias flipped to ", AI_Bias, ". Closing all ", posCount, " positions.");
         CloseAllPositions();
         Sleep(500);
         return;
      }
      
      // EMA crossover reversal: close if EMA crosses against position direction
      double emaF[], emaS[];
      ArraySetAsSeries(emaF, true);
      ArraySetAsSeries(emaS, true);
      CopyBuffer(hEMA_Fast_M1, 0, 0, 3, emaF);
      CopyBuffer(hEMA_Slow_M1, 0, 0, 3, emaS);
      
      bool emaBearishCrossNow = (emaF[1] < emaS[1]) && (emaF[2] >= emaS[2]);
      bool emaBullishCrossNow = (emaF[1] > emaS[1]) && (emaF[2] <= emaS[2]);
      
      if(posType == POSITION_TYPE_BUY && emaBearishCrossNow)
      {
         int posCount = CountOpenPositions();
         Print("[EMA REVERSAL] Bearish EMA cross while LONG. Closing all ", posCount, " positions.");
         CloseAllPositions();
         Sleep(500);
         return;
      }
      if(posType == POSITION_TYPE_SELL && emaBullishCrossNow)
      {
         int posCount = CountOpenPositions();
         Print("[EMA REVERSAL] Bullish EMA cross while SHORT. Closing all ", posCount, " positions.");
         CloseAllPositions();
         Sleep(500);
         return;
      }
   ...
   // === PYRAMIDING: Check if we can add more positions ===
   int openCount = CountOpenPositions();
   if(UsePyramiding)
   {
      if(openCount >= MaxOpenPositions) return;
      
      // For pyramid entries (2nd, 3rd...), use longer cooldown
      int cooldown = (openCount == 0) ? Scalp_CooldownSeconds : Pyramid_CooldownSeconds;
      int scalpElapsed = (int)(currentTime - LastScalpEntry);
      if(scalpElapsed < cooldown) return;
      
      // Existing positions must be in profit before pyramiding
      if(openCount > 0 && !AreExistingPositionsInProfit())
         return;
   }
   else
   {
      // No pyramiding: only enter if no position exists
...
void UpdateAIBias()
{
   ...
   if(raw != "")
   {
      ...
      AI_Bias_Previous = AI_Bias;  // Store previous bias before updating
      ...
      if(confidence >= activeThreshold)
      {
         ...
         // Detect reversal
         if(AI_Bias_Previous != "HOLD" && AI_Bias != AI_Bias_Previous && AI_Bias != "HOLD")
            Print("[AI REVERSAL] Bias changed from ", AI_Bias_Previous, " -> ", AI_Bias);
      }
      else
      {
         AI_Bias = "HOLD";
...
void OpenScalpBuy()
{
   ...
   {
      int posNum = CountOpenPositions();
      Print("[TRADE] Scalp BUY #", posNum, " @ ", ask, " SL:", sl, " TP:", tp, " Ticket:", trade.ResultOrder());
      LastScalpEntry = TimeCurrent();
      TodayTradeCount++;
      PartialTP_Done = false;
   }
}
...
void OpenScalpSell()
{
   ...
   {
      int posNum = CountOpenPositions();
      Print("[TRADE] Scalp SELL #", posNum, " @ ", bid, " SL:", sl, " TP:", tp, " Ticket:", trade.ResultOrder());
      LastScalpEntry = TimeCurrent();
      TodayTradeCount++;
      PartialTP_Done = false;
   }
...
//--------------------------------------------------
bool AreExistingPositionsInProfit()
{
   // All existing positions must have at least Pyramid_MinProfitPoints of profit
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(PositionGetSymbol(i) != _Symbol) continue;
      if(PositionGetInteger(POSITION_MAGIC) != MagicNumber) continue;
      ...
      double profit = 0;
      if(type == POSITION_TYPE_BUY)
         profit = (SymbolInfoDouble(_Symbol, SYMBOL_BID) - openPrice) / _Point;
      else
         profit = (openPrice - SymbolInfoDouble(_Symbol, SYMBOL_ASK)) / _Point;
      
      if(profit < Pyramid_MinProfitPoints)
         return false;  // At least one position not sufficiently in profit
   }
   return true;
}
