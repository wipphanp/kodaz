//+------------------------------------------------------------------+
//|                    ea_5minCandle_scalp_v6_FixedSL_TimeFilter.mq5 |
...
//| ea_5minCandle_scalp_v6_FixedSL_TimeFilter.mq5                    |
//| Copyright 2026                                                   |
//| M5 Candle Scalper v6 — Fixed 18pt SL + Entry Time Window Filter  |
//+------------------------------------------------------------------+
//| CHANGES FROM v4:                                                  |
//|  - SL is now ALWAYS a fixed 18 points on every entry (buy or     |
//|    sell), regardless of ATR/volatility. Buy at 4058 -> SL 4040.  |
//|    Sell at 4058 -> SL 4076. No more ATR-based SL sizing.         |
//|  - Removed hardcoded dollar TP (TakeProfit_Dollars = 12.03) from |
//|    CheckProfitTarget(). TP is now ATR-based only, set at entry   |
//|    and managed by the trailing stop - no dollar-value early exit.|
//|  - Added an M1-confirmation TIME WINDOW filter within the M5     |
//|    candle:                                                        |
//|      * BUY entries are only allowed if confirmation happens      |
//|        within the first Max_Buy_EntryMinutes (default 4) M1 bars |
//|        of the candle - late buys are skipped (reversal risk).    |
//|      * SELL entries require AT LEAST Min_Sell_EntryMinutes       |
//|        (default 2) M1 bars to have elapsed before confirming -   |
//|        too-early sells are skipped (reversal risk).              |
//+------------------------------------------------------------------+
...
//=== SL SETTINGS (NEW - ALWAYS FIXED, NO ATR SIZING) ===
input double Fixed_SL_Points = 18;   // SL is ALWAYS this many points from entry, buy or sell

//=== TP SETTINGS (ATR-based only, hardcoded dollar TP removed) ===
...
//=== ENTRY TIME WINDOW FILTER (NEW) ===
input int Max_Buy_EntryMinutes  = 4; // BUY only allowed if confirmed within this many M1 bars of the M5 candle
input int Min_Sell_EntryMinutes = 2; // SELL only allowed if AT LEAST this many M1 bars have elapsed
...
int OnInit()
{
   ...
   Print("[INIT] M5 Candle Scalper v6 (Fixed 18pt SL + Entry Time Window)");
   Print("[INIT] Fixed SL=", DoubleToString(Fixed_SL_Points,0), "pts (always) | TP=", DoubleToString(ATR_TP_Mult,2), "xATR");
   Print("[INIT] Buy allowed within first ", Max_Buy_EntryMinutes, " M1 bars | Sell requires >= ", Min_Sell_EntryMinutes, " M1 bars");
   Print("[INIT] Broker min stop level: ", SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL), " points");
...
//+------------------------------------------------------------------+
// M1 CONFIRMATION with ENTRY TIME WINDOW FILTER (NEW)
//   BUY  -> only confirmable within the first Max_Buy_EntryMinutes
//           M1 bars of the M5 candle (late buys skipped - reversal risk)
//   SELL -> only confirmable once at least Min_Sell_EntryMinutes M1
//           bars have elapsed (too-early sells skipped - reversal risk)
//+------------------------------------------------------------------+
void CheckM1Confirmation()
{
   ...
   // NEW: asymmetric time-window filter
   if(CandleDirection == "BUY" && M1BarsElapsed > Max_Buy_EntryMinutes)
   {
      CandleDirection = "SKIP";
      Print("[SKIP] BUY too late in candle (", M1BarsElapsed, " min elapsed, max=", Max_Buy_EntryMinutes, ")");
      return;
   }

   if(CandleDirection == "SELL" && M1BarsElapsed < Min_Sell_EntryMinutes)
   {
      // not yet time to confirm a sell - keep waiting, don't skip
      return;
   }

   // generic safety cutoff - never confirm past bar 4 of a 5-candle
   if(M1BarsElapsed > 4)
   {
      CandleDirection = "SKIP";
...
//+------------------------------------------------------------------+
// SL is now ALWAYS fixed at Fixed_SL_Points - no ATR sizing, no
// per-direction variation. TP remains ATR-based (dollar TP removed).
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
// Ensures SL/TP distances respect the broker's minimum stop level,
// preventing "invalid stops" rejections.
//+------------------------------------------------------------------+
double EnforceMinStopDistance(double dist)
{
   ...
   if(minDist > 0 && dist < minDist)
      return minDist * 1.2; // small safety margin
   return dist;
}
...
void ExecuteBuy()
{
   ...
   sl_dist = EnforceMinStopDistance(sl_dist);
   tp_dist = EnforceMinStopDistance(tp_dist);
   ...
   if(!trade.Buy(lot, _Symbol, ask, sl, tp, "M5v6 BUY"))
      Print("[ERROR] BUY failed: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
   else
   {
      TradeOpenThisCandle = true;
      TodayTrades++;
      Print("[ENTRY] BUY ", lot, " lots | SL:", sl, " TP:", tp, " | M1 bars at entry: ", M1BarsElapsed);
   }
}
...
void ExecuteSell()
{
   ...
   if(!trade.Sell(lot, _Symbol, bid, sl, tp, "M5v6 SELL"))
      Print("[ERROR] SELL failed: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
   else
   {
      TradeOpenThisCandle = true;
      TodayTrades++;
      Print("[ENTRY] SELL ", lot, " lots | SL:", sl, " TP:", tp, " | M1 bars at entry: ", M1BarsElapsed);
   }
}

//+------------------------------------------------------------------+
// Hardcoded dollar TP REMOVED. Only the ATR-based dynamic target
// (2x ATR) remains as a runtime profit check; the order-level TP
// set at entry (ATR_TP_Mult) still governs the broker-side TP.
//+------------------------------------------------------------------+
