//+------------------------------------------------------------------+
//| ea_5minCandle_scalp_v6_Fixed18SL_COMPLETE_PATCHED.mq5             |
//| Original strategy with broker stop/freeze validation patch        |
//+------------------------------------------------------------------+
...
long GetBrokerStopsLevelPoints()
{
   return SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
}

long GetBrokerFreezeLevelPoints()
{
   return SymbolInfoInteger(_Symbol, SYMBOL_TRADE_FREEZE_LEVEL);
}
...
void GetSafeSLTP(bool isBuy, double entryPrice, double &sl, double &tp, double &safeSLDist, double &safeTPDist)
{
   ...
   safeSLDist = MathMax(reqSLDist, minStopDist + extraBuf);
   safeTPDist = MathMax(reqTPDist, minStopDist + extraBuf);
...
bool PlaceBuyWithFallback(double lot, double ask, double sl, double tp)
{
   ...
   {
      Print("[ERROR] PositionModify BUY failed: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
      return false;
   }
...
bool PlaceSellWithFallback(double lot, double bid, double sl, double tp)
{
   ...
   if(!trade.PositionModify(_Symbol, sl, tp))
   {
      Print("[ERROR] PositionModify SELL failed: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
      return false;
   }
...
int OnInit()
{
   ...
   Print("[INIT] M5 Candle Scalper v6 patched");
   Print("[INIT] Fixed SL=" + DoubleToString(Fixed_SL_Points, 0) + "pts | TP=" + DoubleToString(ATR_TP_Mult, 2) + "xATR");
   Print("[INIT] Broker stop level pts=", GetBrokerStopsLevelPoints(), " freeze level pts=", GetBrokerFreezeLevelPoints());
...
void ExecuteBuy()
{
   ...
   Print("[BUY CHECK] Ask=", ask,
         " | lot=", lot,
         " | SL=", sl,
         " | TP=", tp,
         " | stopLevelPts=", GetBrokerStopsLevelPoints(),
         " | freezeLevelPts=", GetBrokerFreezeLevelPoints(),
         " | safeSLDist=", safeSLDist,
         " | safeTPDist=", safeTPDist);
...
void CheckProfitTarget()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      double priceCurrent = (type == POSITION_TYPE_BUY) ? SymbolInfoDouble(_Symbol, SYMBOL_BID)
                                                        : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double profitPoints = (type == POSITION_TYPE_BUY) ? (priceCurrent - priceOpen) / _Point
                                                        : (priceOpen - priceCurrent) / _Point;
...
void UpdateAIBias()
{
   ...
   if(conf >= AI_ConfidenceThreshold)
   {
      AI_Bias = sig;
      AI_Confidence = conf;
      Print("[AI] ", AI_Bias, " (", conf, ")");
   }
   else
   {
      AI_Bias = "NONE";
      Print("[AI] Low confidence. No filter.");
   }
}
