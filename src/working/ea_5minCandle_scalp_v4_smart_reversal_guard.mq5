...
//=== PROFIT MANAGEMENT SIMPLIFICATION ===
// NOTE: The previous hardcoded dollar TP has been removed.
// NOTE: The previous redundant floating 2x ATR profit close has also been removed.
// NOTE: Profit management is now simplified to broker-side ATR TP at entry,
// trailing stop management, candle-end management, and RSI emergency guard exits.
...
input double RSI_BuyAbove = 51.0; // Existing lower BUY threshold remains unchanged.
input double RSI_BuyBelow = 75.0; // NEW: BUY direction is allowed only while RSI stays below this upper cap.
input double RSI_SellBelow = 47.0; // Existing upper SELL threshold remains unchanged.
input double RSI_SellAbove = 36.0; // NEW: SELL direction is allowed only while RSI stays above this lower floor.

//=== REVERSAL & LOSS MINIMIZATION LOGIC ===
input bool UseSmartReversalGuard = true;
input double SmartLossCut_Dollars = 60.0; // emergency cut if floating loss exceeds this amount
input int LateCandleMinuteThreshold = 4;   // BUY late in M5 candle is treated as exhaustion-sensitive
input int EarlyCandleMinuteThreshold = 2;  // SELL early in M5 candle is treated as snapback-sensitive
input double CandleHalfBreakBufferPoints = 5.0; // buffer around half-candle level before reversal action
input bool AllowImmediateReverseOnGuard = true;
...
bool SmartGuardTriggeredThisTick = false;
...
Print("[INIT] M5 Candle Scalper v4 (RSI Guard + Simplified TP Update)");
...
Print("[INIT] BUY RSI band: >" + DoubleToString(RSI_BuyAbove, 1) + " and <" + DoubleToString(RSI_BuyBelow, 1));
Print("[INIT] SELL RSI band: <" + DoubleToString(RSI_SellBelow, 1) + " and >" + DoubleToString(RSI_SellAbove, 1));
Print("[INIT] Smart guard loss cut=$" + DoubleToString(SmartLossCut_Dollars, 2) + " | Late minute>=" + IntegerToString(LateCandleMinuteThreshold) + " | Early minute<=" + IntegerToString(EarlyCandleMinuteThreshold));
...
if(HasPosition())
{
   // NEW: enforce the RSI emergency guard before any other position management.
   // This closes trades immediately if an open BUY drifts into RSI >= 75
   // or an open SELL drifts into RSI <= 36.
   EnforceRSIGuardCloses();

   // NEW: only continue to TP/trailing management if the RSI guard did not already close the position.
   if(HasPosition())
   {
      CheckProfitTarget();
...
// NEW: BUY now requires RSI to stay inside the approved momentum band (>51 and <75).
if(bullish >= 2 && rsi5[0] > RSI_BuyAbove && rsi5[0] < RSI_BuyBelow && buyScore >= MTF_MinScore)
direction = "BUY";
// NEW: SELL now requires RSI to stay inside the approved momentum band (<47 and >36).
else if(bearish >= 2 && rsi5[0] < RSI_SellBelow && rsi5[0] > RSI_SellAbove && sellScore >= MTF_MinScore)
direction = "SELL";
}
else
{
// NEW: same RSI band rule applied to the non-scoring branch so both entry modes behave consistently.
if(bullish >= 2 && m5_bullish && rsi5[0] > RSI_BuyAbove && rsi5[0] < RSI_BuyBelow && m15_bullish && h1_bullish)
direction = "BUY";
else if(bearish >= 2 && m5_bearish && rsi5[0] < RSI_SellBelow && rsi5[0] > RSI_SellAbove && m15_bearish && h1_bearish)
direction = "SELL";
}
...
//+------------------------------------------------------------------+
void EnforceRSIGuardCloses()
{
   // NEW: This guard watches the live M5 RSI after a trade is already open.
   // If BUY reaches/overruns RSI 75, or SELL reaches/underruns RSI 36,
   // the EA exits immediately instead of waiting for normal candle-end or TP logic.
   double rsi5[];
   ArraySetAsSeries(rsi5, true);
   if(CopyBuffer(hRSI_M5, 0, 0, 1, rsi5) <= 0) return;

   double currentRSI = rsi5[0];
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      // NEW: BUY safety exit once RSI enters the forbidden overbought zone.
      if(type == POSITION_TYPE_BUY && currentRSI >= RSI_BuyBelow)
      {
         if(trade.PositionClose(ticket))
         {
            ...
            DirectionDecided = false;
            CandleDirection = "SKIP";
            Print("[RSI GUARD] Closed BUY immediately. RSI=", DoubleToString(currentRSI, 2),
                  " >= ", DoubleToString(RSI_BuyBelow, 2), " | P/L=$", DoubleToString(profit, 2));
         }
         else
         {
            Print("[ERROR] RSI guard BUY close failed: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
         }
         continue;
      }

      // NEW: SELL safety exit once RSI enters the forbidden oversold zone.
      if(type == POSITION_TYPE_SELL && currentRSI <= RSI_SellAbove)
      {
         if(trade.PositionClose(ticket))
         {
            if(profit > 0) TodayWins++; else TodayLosses++;
            ...
            DirectionDecided = false;
            CandleDirection = "SKIP";
            Print("[RSI GUARD] Closed SELL immediately. RSI=", DoubleToString(currentRSI, 2),
                  " <= ", DoubleToString(RSI_SellAbove, 2), " | P/L=$", DoubleToString(profit, 2));
         }
         else
         {
            Print("[ERROR] RSI guard SELL close failed: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
         }
      }
   }
}
...
//+------------------------------------------------------------------+
int GetCurrentM5MinuteElapsed()
{
   datetime candleTime = iTime(_Symbol, PERIOD_M5, 0);
   if(candleTime <= 0) return 0;
   int elapsed = (int)((TimeCurrent() - candleTime) / 60);
...
//+------------------------------------------------------------------+
bool GetRealtimeM5State(double &m5Open, double &curPrice, double &halfLevel, double &candleMovePoints, double &rsiNow)
{
   m5Open = iOpen(_Symbol, PERIOD_M5, 0);
   if(m5Open <= 0) return false;

   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   curPrice = (bid + ask) / 2.0;
   candleMovePoints = (curPrice - m5Open) / _Point;
   halfLevel = m5Open + (candleMovePoints * 0.5 * _Point);

   double rsi5[];
   ArraySetAsSeries(rsi5, true);
   if(CopyBuffer(hRSI_M5, 0, 0, 1, rsi5) <= 0) return false;
   rsiNow = rsi5[0];
   return true;
}

//+------------------------------------------------------------------+
string EvaluateReverseDirectionNow()
{
...
//+------------------------------------------------------------------+
void EnforceSmartReversalGuard()
{
   double m5Open, curPrice, halfLevel, candleMovePoints, rsiNow;
   if(!GetRealtimeM5State(m5Open, curPrice, halfLevel, candleMovePoints, rsiNow)) return;

   int minuteElapsed = GetCurrentM5MinuteElapsed();
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double buffer = CandleHalfBreakBufferPoints * _Point;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      bool closeForLossCap = (profit <= -MathAbs(SmartLossCut_Dollars));
      bool closeForLateBuyFailure = false;
      bool closeForEarlySellFailure = false;
      string reverseDirection = "SKIP";

      if(type == POSITION_TYPE_BUY)
      {
         if(minuteElapsed >= LateCandleMinuteThreshold && rsiNow >= 60.0 && candleMovePoints > 0)
         {
            double buyFailLevel = m5Open + (candleMovePoints * 0.5 * _Point);
            if(bid < buyFailLevel - buffer)
            {
               closeForLateBuyFailure = true;
               reverseDirection = EvaluateReverseDirectionNow();
            }
         }
      }
      else if(type == POSITION_TYPE_SELL)
      {
         if(minuteElapsed <= EarlyCandleMinuteThreshold && rsiNow <= 43.0 && candleMovePoints < 0)
         {
            double sellFailLevel = m5Open + (candleMovePoints * 0.5 * _Point);
            if(ask > sellFailLevel + buffer)
            {
               closeForEarlySellFailure = true;
               reverseDirection = EvaluateReverseDirectionNow();
            }
         }
      }

      if(closeForLossCap || closeForLateBuyFailure || closeForEarlySellFailure)
      {
         string reason = closeForLossCap ? "LOSS CAP" : (closeForLateBuyFailure ? "LATE BUY FAILURE" : "EARLY SELL FAILURE");
         ...
         if(closed)
         {
            ...
            DirectionDecided = false;
            CandleDirection = "SKIP";
            SmartGuardTriggeredThisTick = true;
            Print("[SMART GUARD] Closed ", EnumToString(type), " reason=", reason, " | minute=", minuteElapsed, " | RSI=", DoubleToString(rsiNow,2), " | P/L=$", DoubleToString(profit,2));

            if(AllowImmediateReverseOnGuard)
            {
               if(type == POSITION_TYPE_BUY && reverseDirection == "SELL" && !HasPosition())
               {
                  Print("[SMART GUARD] Reversing BUY -> SELL after failed late-candle continuation.");
                  ExecuteSell();
               }
               else if(type == POSITION_TYPE_SELL && reverseDirection == "BUY" && !HasPosition())
               {
                  Print("[SMART GUARD] Reversing SELL -> BUY after failed early-candle continuation.");
                  ExecuteBuy();
               }
            }
         }
         ...
         {
            Print("[ERROR] Smart guard close failed: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
         }
      }
   }
}
...
{
   // SIMPLIFIED: This function is intentionally kept as a no-op.
   // Reason 1: The trade already gets an ATR-derived broker-side TP in ExecuteBuy/ExecuteSell.
   // Reason 2: The old hardcoded dollar TP ($12.03) caused fixed-currency exits that did not scale well.
   // Reason 3: The old floating 2x ATR close overlapped with the broker TP and could cut winners early.
   // Remaining exit stack is now cleaner: broker-side ATR TP, trailing stop, candle-end logic, and RSI guard exits.
   return;
}
...
{
Print("[CANDLE END HOLD] Loss beyond soft candle-end threshold. Position left to SL / smart guard / RSI guard.");
}
}
}
