...
//=== SL/TP SETTINGS ===
input double Fixed_SL_Points = 1500;   // [MOD] Static SL distance fixed at 1500 points on entry        // Fixed SL distance in points (e.g. price 4060 → SL 4042)
...
{
   // Static SL: always Fixed_SL_Points, converted to price units.
   sl_dist = Fixed_SL_Points * _Point;   // [MOD] Always convert 1500 points into price units for SL

   // Keep existing TP logic based on ATR and ATR_TP_Mult / Max/Min TP points.
   double atrPts = GetATR() / _Point;   // [INFO] ATR-based TP logic preserved from original EA
   double targetPts = atrPts * ATR_TP_Mult;
   if(targetPts > Max_TP_Points) targetPts = Max_TP_Points;
   if(targetPts < Min_TP_Points) targetPts = Min_TP_Points;

   tp_dist = targetPts * _Point;        // [INFO] TP distance still dynamic based on ATR_TP_Mult / Min/Max TP

   Print("[SLTP] Static SL=", DoubleToString(Fixed_SL_Points,0), "pts | TP ATR=",
         DoubleToString(ATR_TP_Mult,2), "x | TP_pts=", IntegerToString((int)targetPts));
}
...
void ExecuteBuy()
{
   ...
   GetSLTP(sl_dist, tp_dist);           // [MOD] Fetch static 1500-pt SL and ATR-based TP distances

   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double sl = NormalizeDouble(ask - sl_dist, _Digits);   // [MOD] Place SL exactly 1500 points below entry
   double tp = NormalizeDouble(ask + tp_dist, _Digits);   // [INFO] TP unchanged (ATR-based)

   double lot = CalcLot(sl_dist);      // [MOD] Position size computed using 1500-pt SL distance
...
void ExecuteSell()
{
   ...
   double sl = NormalizeDouble(bid + sl_dist, _Digits);   // [MOD] Place SL exactly 1500 points above entry
   double tp = NormalizeDouble(bid - tp_dist, _Digits);   // [INFO] TP unchanged (ATR-based)
...
{
   // Profit management handled purely via trailing stop and candle-end logic.
   // No fixed dollar TP or ATR-based hard profit target.
   return;
}
